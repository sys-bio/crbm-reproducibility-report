...
\begin{todolist}
  \item[\VAR{rubric2[1][0]}] Model/source code is available at a public repository or researcher's web site
  \begin{todolist}
    \item[\absent] Prohibitive license provided
    \item[\absent] Permissive license provided
    \item[\absent] Open-source license provided
  \end{todolist}
  \item[\VAR{rubric2[1][1]}] All initial conditions and parameters are provided
  \item[\VAR{rubric2[1][2]}] All simulation experiments are fully defined (events listed, collection times and measurements specified, algorithms provided, simulator specified, etc.)
\end{todolist}
\end{tcolorbox}
...
\begin{todolist}
    \item[
        %% if rubric2[2][0] == 'Insufficient'
            \absent
        %% else
            \present
        %% endif
    ] Define context clearly: \VAR{rubric2[2][0]}
    \item[
        %% if rubric2[2][1] == 'Insufficient'
            \absent
        %% else
            \present
        %% endif
    ] Use appropriate data: \VAR{rubric2[2][1]}
    \item[
        %% if rubric2[2][2] == 'Insufficient'
            \absent
        %% else
            \present
        %% endif
    ] Evaluate within context: \VAR{rubric2[2][2]}
    \item[
        %% if rubric2[2][3] == 'Insufficient'
            \absent
        %% else
            \present
        %% endif
    ] List limitations explicitly: \VAR{rubric2[2][3]}
    \item[
        %% if rubric2[2][4] == 'Insufficient'
            \absent
        %% else
            \present
        %% endif
    ] Use version control: \VAR{rubric2[2][4]}
    \item[
        %% if rubric2[2][5] == 'Insufficient'
            \absent
        %% else
            \present
        %% endif
    ] Document adequately: \VAR{rubric2[2][5]}
    \item[
        %% if rubric2[2][6] == 'Insufficient'
            \absent
        %% else
            \present
        %% endif
    ] Conform to standards: \VAR{rubric2[2][6]}
\end{todolist}
\end{tcolorbox}
...
\begin{todolist}
    \item[\VAR{rubric2[3][0]}] Model and its simulations could be repeated using provided declarative or procedural code
    \item[\VAR{rubric2[3][1]}] Model and its simulations could be reproduced
\end{todolist}
\end{tcolorbox}
